...
Из `.puml` aact собирает граф: элементы, границы, связи. Посмотреть, что
именно распарсилось:
...
имя по конвенции (запасной вариант для легаси- и AI-сгенерённых диаграмм без тегов).

| Роль            | Тег                   | Имя (запасной вариант)                                                              | Кто читает             |
| --------------- | --------------------- | ----------------------------------------------------------------------------------- | ---------------------- |
...
`npx aact init` кладёт такую же стартовую модель (с пояснительными комментариями) —
...
суффиксу `_repo`. Это и есть распознавание по имени из таблицы выше.
